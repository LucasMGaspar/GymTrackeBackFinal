...
export function HistoryClient({ student, sessions, currentUserId, currentUserRole }: Props) {
  ...
  const calculateStats = () => {
    ...
    // Calculate adherence (% of days with workouts in period)
    const daysInPeriod = filterPeriod === 'all' ? 30 : parseInt(filterPeriod.replace('days', ''));
    const adherencePercent = Math.round((total / daysInPeriod) * 100);

    return { total, avgExercises, avgDuration, adherencePercent };
  };
    ...
    <div className="space-y-6">
      ...
      {stats.total > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="bg-white rounded-lg shadow-sm p-4 text-center">
            <div className="text-2xl font-bold text-green-600">{stats.adherencePercent}%</div>
